		...
		namespace Revelation
		{
						...
						while (ini.read_value())
						{
							if (ini.is_value("String"))
							{
								if (parse(ini.get_value_string(), key, val))
								{
									...
									{
										Print::Info("языкова€ строка \"" + key + "\" уже определена. ќна будет перезаписана.", "");
									}
									else
									{
										Print::Debug("ƒобавление €зыковой строки \"" + key + "\" ...", "");
									}
			...
			const wstring& Language::lang(const string key, const wstring defaultStr)
			{
				if (languages.find(key) == languages.end())
				{
					...
					Print::Debug(string("языкова€ строка \"").append(key).append("\" не была определена, вместо этого используетс€ default."), "");
				}
